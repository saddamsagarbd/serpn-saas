<div 
    ...
    <div x-ref="modalContainer" class="bg-white rounded-2xl shadow-xl border border-slate-200 w-full max-w-md overflow-hidden" @click.away="openModal = false">
        <div class="px-6 py-4 bg-slate-50 border-b border-slate-100 flex justify-between items-center">
            <h3 class="text-sm font-bold text-slate-800">Add Warehouse</h3>
            <button @click="openModal = false" class="text-slate-400 hover:text-slate-600">✕</button>
        </div>
        <form @submit.prevent="submitForm" class="p-6 space-y-4">
            <div class="space-y-1">
                <label class="text-[10px] font-bold text-slate-500 uppercase">Warehouse Name *</label>
            ...
            <div class="space-y-1">
                <label class="text-[10px] font-bold text-slate-500 uppercase">Warehouse Address</label>
                <input type="text" x-model="formData.address" class="w-full px-3 py-2 text-xs border rounded-xl focus:outline-none focus:border-indigo-500" placeholder="e.g., Spring 2026 / EF26" required>
            </div>
            <div class="space-y-1">
                <input type="checkbox" x-model="formData.isDefault" id="is_default">
                <label class="text-[10px] font-bold text-slate-500 uppercase">Is Default</label>
            </div>
