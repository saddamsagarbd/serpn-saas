    ...
    <div class="bg-white rounded-2xl border border-slate-200 max-w-md w-full shadow-xl overflow-hidden" @click.away="openModal = false">
        ...
        <form @submit.prevent="saveBrand()" class="p-6 space-y-4">
            ...
            <div class="space-y-1">
                ...
                <input type="text" x-model="brandData.name" required placeholder="e.g.  Multi Brand, Spartan Shield" class="w-full px-3 py-2 text-xs border border-slate-200 rounded-xl focus:outline-none focus:border-indigo-500 font-medium text-slate-800">
            </div>         
