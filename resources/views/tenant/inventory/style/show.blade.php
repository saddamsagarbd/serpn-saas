...
<div class="max-w-5xl mx-auto space-y-6 my-6">
    ...
    <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex justify-between items-center">
        ...
        <div class="flex items-center gap-2">
            <a href="{{ route('tenant.inventory.styles.edit', $style->id) }}" class="px-4 py-2 bg-slate-100 text-slate-700 text-xs font-bold rounded-xl hover:bg-slate-200 transition">
                Modify Style
            </a>
            <!-- PDF Export Button -->
            <a href="{{ route('tenant.inventory.styles.export-pdf', $style->id) }}" target="_blank" class="px-4 py-2 bg-red-600 hover:bg-red-700 text-white text-xs font-bold rounded-xl shadow-sm transition inline-flex items-center gap-1.5">
