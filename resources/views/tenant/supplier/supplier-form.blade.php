            ...
            <form action="{{ isset($supplier) ? route('tenant.purchase.suppliers.update', $supplier->id) : route('tenant.purchase.suppliers.store') }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            <label class="block text-xs font-semibold text-gray-600 mb-1">Supplier Type <span class="text-red-500">*</span></label>
                            <select name="supplier_type" required class="w-full border border-gray-300 rounded-lg text-xs p-2.5">
                                <option value="">-- Select Type --</option>
                                @foreach($supplierTypes as $type => $label)
                                    <option value="{{ $type }}" @selected(old('supplier_type', $supplier->supplier_type ?? 'general') == $type)>{{ $label }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div>
                ...
                <div class="flex justify-end space-x-2 pt-4 border-t border-gray-100 gap-2">
                    <a href="{{ route('tenant.purchase.suppliers.index') }}" class="px-4 py-2 border border-gray-300 rounded-lg text-xs font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none">
                        Cancel
                    </a>
